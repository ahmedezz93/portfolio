...
<head>
    ...
    <title>{{ $settings->change_name ?? '' }}</title>
                        ...
                        <div class="app-brand justify-content-center mb-4 mt-2">
                            <a href="" class="app-brand-link gap-0">
                                <span class="app-brand-logo demo">
                                    <img src="{{ $settings->logo_url ?? '' }}" alt="" class="img-fluid" style="width:55height: 55px;"/>
                                </span>
                            </a>
                        </div>

                        @if (Session::has('message'))
                          <p class="alert {{ Session::get('alert-class', 'alert-danger') }}">{{ Session::get('message') }}</p> @endif
                        <!-- /Logo -->
                        <h6 class="mb-1
        pt-2">    {{ $settings->change_name ?? ''}}</h6>
    ...
    <form class="mb-3" action="{{ route('admin.login') }}" method="POST">
        ...
        <div class="mb-3">
            <div class="form-check">
                ...
                <label class="form-check-label" for="remember-me"> {{__('login.remember')}} </label>
            </div>
        </div>
        <div class="mb-3">
